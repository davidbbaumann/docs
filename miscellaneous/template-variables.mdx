...
Variables are placeholders that are automatically replaced with real values from records when using templates or executing an AI prompt. They always follow the pattern `{{namespace.field}}` and can optionally include a fallback value: `{{candidate.first_name\|"Candidate"}}`.
...
| `{{candidate.first_name|"Candidate"}}` | Fallback if the field is empty |
